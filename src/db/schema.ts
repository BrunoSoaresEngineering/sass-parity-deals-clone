...
export const TierEnum = pgEnum(
  'tier',
  Object.keys(subscriptionTiers) as [keyof typeof subscriptionTiers],
);
...
export const ProductTable = pgTable(
  'products',
  {
    ...
    clerkUserId: text('clerk_user_id')
      .notNull()
      .references(() => UserSubscriptionTable.clerkUserId, { onDelete: 'cascade' }),
...
export const ProductCustomizationTable = pgTable('product_customizations', {
  ...
  backgroundColor: text('backgorund_color').notNull().default('hsl(193, 82%, 31%)'),
...
export const productRelations = relations(
  ...
  ({ one, many }) => ({
    ...
    clerkUserId: one(UserSubscriptionTable, {
      fields: [ProductTable.clerkUserId],
      references: [UserSubscriptionTable.clerkUserId],
    }),
  }),
);
...
export const UserSubscriptionRelations = relations(UserSubscriptionTable, ({ many }) => ({
